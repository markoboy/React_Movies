...
import ResultCount from '@components/ResultList/ResultCount';
import ResultItemPopOut from '@components/ResultList/ResultItemPopOut';
  ...
  return (
    <>
      ...
      <section className="section section--padding-bottom-only">
        <ul className="result-list">{movies.map(getResultListItems)}</ul>
      </section>
    </>
  );
}
...
export default connect(mapStateToProps)(memo(withWrapper(ResultListContainer)));
